...
from transformers.models.bert.modeling_bert import BertPreTrainedModel # Abandon pre-trained network
...
class ModalEmbeddings(nn.Module):
    ...
    def __init__(self, config, modal_hidden_size, encoder, embeddings, cvae=False):
        """
        ...
        self.proj_embeddings = nn.Linear(modal_hidden_size, config.hidden_size) # Project data from 3072 dimensions to 768
    ...
    def forward(
      ...
      # print ("ModalEmbeddings_input_modal_SIZE",input_modal.size()) #ModalEmbeddings_input_modal_SIZE ([15, 9, 224, 224])
        if self.cvae:
            mu, logvar, reconstructed_mean, reconstructed_kappa, out = self.encoder(input_modal, text_embeddings=text_embeddings)
        else:
            out = self.encoder(input_modal)
        token_embeddings = self.proj_embeddings(out)
      ...
      #  print ("embeddings_2",embeddings.shape) #embeddings_2 torch.Size([15, 5, 768])
       ...
       # print ("embeddings_3",embeddings.shape) #embeddings_3 torch.Size([15, 5, 768])
        if self.cvae:
            return mu, logvar, reconstructed_mean, reconstructed_kappa, embeddings
        else:
            return embeddings
...
class MMBTModel(nn.Module, ModuleUtilsMixin):
    def __init__(self, config, mmbt_config, cvae):
        super().__init__()
        self.config = config
        ...
        self.modal_encoder = ModalEmbeddings(config,
                                             ...
                                             cvae=cvae)
        self.cvae = cvae
        # Store reference to image encoder for CVAE access
        if self.cvae:
            self.image_encoder = mmbt_config.encoder
    """
    ...
    def forward(
        ...
        # Compute text embeddings for CVAE conditioning
        if token_type_ids is None:
        ...
        # Get text representation for CVAE (pool the embeddings to get a single vector per sample)
        # Average pooling over sequence dimension: (B, seq_len, hidden_size) -> (B, hidden_size)
        txt_embeddings_pooled = txt_embeddings.mean(dim=1) if txt_embeddings.dim() > 2 else txt_embeddings
        
        encoder_output = self.modal_encoder(
            input_modal,
            start_token=modal_start_tokens,
            end_token=modal_end_tokens,
            position_ids=modal_position_ids,
            token_type_ids=modal_token_type_ids,
            text_embeddings=txt_embeddings_pooled if self.cvae and self.image_encoder.cvae_encoder.text_embedding_dim is not None else None,
        )
        if self.cvae:
            mu, logvar, reconstructed_mean, reconstructed_kappa, modal_embeddings = encoder_output
        else:
            modal_embeddings = encoder_output
        ...
        sequence_output = encoder_outputs[0] # sequence_output dimension will be [batch_size, sequence_length, hidden_size]. It contains the encoder's final hidden states and can be used for downstream tasks like classification or generation.
        ...
        pooled_output = self.transformer.pooler(sequence_output) # pooled_output can be used for downstream tasks like text classification; input to a fully-connected layer to generate classification results.
        ...
        if self.cvae:
            ...
            ), mu, logvar, reconstructed_mean, reconstructed_kappa
        else:
            return BaseModelOutputWithPooling(
                last_hidden_state=sequence_output,
                pooler_output=pooled_output,
...
#class MMBTForClassification(nn.Module):
    ...
    **hidden_states**: (`optional`, returned when ``output_hidden_states=True``) list of ``torch.FloatTensor`` (one for
    ...
    def __init__(self, config, mmbt_config, cvae): 
        ...
        self.mmbt = MMBTModel(config, mmbt_config, cvae)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(config.hidden_size, config.num_labels)
        self.cvae = cvae
        # Store image encoder for access to CVAE components
        if self.cvae:
            self.image_encoder = mmbt_config.encoder
    ...
    def forward(
        ...
        if labels is not None:
            if self.num_labels == 1:
                ...
                loss = loss_fct(logits.view(-1, self.num_labels), labels.view(-1)) # logits.view(-1, self.num_labels): loss dimension is m*n/9; reshape to 1D with view(-1)
        ...
        if not return_dict:
            ...
            if not self.cvae:
                ...
                return ((loss, mu, logvar, reconstructed_mean, reconstructed_kappa) + output) if loss is not None else output
        ...
        # np.set_printoptions(threshold=np.inf) # Fixed truncation issue
        # Attention_12heads=outputs.attentions
        # numpy_Attention_12heads = [tensor.detach().cpu().numpy() for tensor in Attention_12heads]
        
        # print ("numpy_Attention_12heads",numpy_Attention_12heads) # When visualizing attention
