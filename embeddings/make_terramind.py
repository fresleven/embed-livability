"""
Generate terramind/*.npz files using rs-embed's TerraMind model.

Each file is named  terramind_{grid_id}_{year}.npz  and stores a grid
embedding produced by TerraMind from Sentinel-2 L2A imagery fetched via GEE.

TerraMind uses COPERNICUS/S2_SR_HARMONIZED (12 bands), TemporalSpec.range,
and outputs a ViT patch-token grid (typically 14×14 for the default 224-px
image at patch-size 16).

Usage
-----
    python make_terramind.py [options]
...
-------
  ...
  --out-dir      DIR   Output folder (default: terramind)
...
def fetch_one(
    ...
    out_path = os.path.join(out_dir, f"terramind_{grid_id}_{year}.npz")
    ...
    band_names = np.array([f"T{i:03d}" for i in range(n_bands)], dtype="<U4")
...
def main(argv: list[str] | None = None) -> None:
    p = argparse.ArgumentParser(
        description="Generate terramind embedding .npz files via rs-embed."
    )
    add_common_args(p)
    p.add_argument("--out-dir", default="terramind",
                   help="Output directory (default: terramind)")
